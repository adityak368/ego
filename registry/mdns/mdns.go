...
func (r *mdnsRegistry) Register(entry registry.Entry) error {
	...
	log.Printf("[MDNS-Registry]: Registered service '%s'", entry.Name)
	r.server = server
	return nil
}
...
func (r *mdnsRegistry) Deregister(serviceName string) error {
	r.server.Shutdown()
	log.Printf("[MDNS-Registry]: Deregistered service '%s'", serviceName)
	return nil
}
			...
			case entry := <-r.entriesChan:
				if entry != nil {
					...
					for _, ip := range entry.AddrIPv4 {
						...
						log.Printf("[MDNS-Registry]: Discovered service %s version {%s} at %s", endpoint.Name, endpoint.Version, endpoint.Address)
